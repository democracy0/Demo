...
TELEGRAM_BOT_TOKEN = '7003192552:AAF8OLRd2cKv5T9geHnxDVKGOWWWSCQP8fo'  # Replace with your bot token
ADMIN_USER_ID = 7161052001
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*❄️ WELCOME TO @OneTwoThreeSK ULTIMATE UDP FLOODER ❄️*\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Bhai, tere paas toh coins nahi hai! Pehle admin ke paas ja aur coins le aa. 😂 DM:- @OneTwoThreeSK*",
            parse_mode='Markdown'
        )
        return
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ @OneTwoThreeSK VIP DDOS Bot Help Menu 🛠️*\n\n"
        ...
        "- *Agar koi dikkat aaye toh admin ko contact karo: @OneTwoThreeSK*\n\n"
